...
const ToolItem = ({ icon, title, url, slug, color }: ToolItemProps) => {
  return (
    <div
      className={cn(
        "group flex items-center mb-5 p-3,5 border rounded-xl transition-all cursor-pointer",
      ...
      <Link href={url} className="w-full">
        <div className="flex items-center justify-between ">
          <div className="flex items-center">
            <div
              className={cn(
                "mr-6 rounded-lg p-1 w-12 h-12  relative flex justify-center"
              )}
            >
              <div className={cn(toolItemColorVariants({ color: slug }))}></div>
